...
function OfficialDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const unpaid = totalChallans - paid;
    const collected = challans
      .filter((c) => c.status === "paid")
      .reduce((sum, c) => sum + Number(c.fine_amount || 0), 0);
    return { totalViolations, totalChallans, paid, unpaid, collected };
  }, [violations, challans]);
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        <Box>
          ...
          <Typography variant="body1" color="text.secondary">
            Monitor violations across the city, generate challans and track enforcement performance.
          </Typography>
        </Box>
      </Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid item xs={12} sm={6} md={2.4}>
          <Paper elevation={3} sx={{ p: 2.5, borderRadius: 3, display: "flex", gap: 1.5 }}>
            <Box
              sx={{
                width: 38,
                height: 38,
            ...
            >
              <LocalPoliceIcon fontSize="small" sx={{ color: "#0f172a" }} />
            </Box>
            <Typography variant="caption" color="text.secondary">
              Total Violations (query)
            </Typography>
            <Typography variant="h5">{stats.totalViolations}</Typography>
          </Paper>
        </Grid>
        <Grid item xs={12} sm={6} md={2.4}>
          <Paper elevation={3} sx={{ p: 2.5, borderRadius: 3, display: "flex", gap: 1.5 }}>
            ...
            >
              <ReceiptLongIcon fontSize="small" sx={{ color: "#0f172a" }} />
            </Box>
            <Typography variant="caption" color="text.secondary">
              Total Challans
            </Typography>
            <Typography variant="h5">{stats.totalChallans}</Typography>
          </Paper>
        </Grid>
        <Grid item xs={12} sm={6} md={2.4}>
          <Paper elevation={3} sx={{ p: 2.5, borderRadius: 3, display: "flex", gap: 1.5 }}>
            ...
            >
              <PaidIcon fontSize="small" sx={{ color: "#15803d" }} />
            </Box>
            <Typography variant="caption" color="text.secondary">
              Paid Challans
            </Typography>
        ...
        <Grid item xs={12} sm={6} md={2.4}>
          <Paper elevation={3} sx={{ p: 2.5, borderRadius: 3, display: "flex", gap: 1.5 }}>
            ...
            >
              <WarningAmberIcon fontSize="small" sx={{ color: "#b91c1c" }} />
            </Box>
            <Typography variant="caption" color="text.secondary">
              Pending Challans
            </Typography>
        ...
        <Grid item xs={12} sm={6} md={2.4}>
          <Paper elevation={3} sx={{ p: 2, borderRadius: 3 }}>
            ...
            <Typography variant="h5">₹{stats.collected.toFixed(0)}</Typography>
          </Paper>
        </Grid>
      </Grid>
